...
internal class NetworkRepositoryImpl(
    ...
    override suspend fun getMovies(
        language: String,
        page: Int
    ): Outcome<PopularMoviesResponse, BaseError> {
        return try {
            Outcome.Success(remoteDataSource.getMovies(language, page))
        } catch (t: Throwable) {
            Outcome.Error(networkErrorHandler.handleError(t))
        }
    }

    override suspend fun getGenres(language: String): Outcome<GenreResponse, BaseError> {
        return try {
            Outcome.Success(remoteDataSource.getGenres(language))
        } catch (t: Throwable) {
            Outcome.Error(networkErrorHandler.handleError(t))
        }
    }
