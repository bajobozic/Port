...
import com.bajobozic.movies_ui.PlatformProgressIndicator
import com.bajobozic.storage.domain.model.Movie
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.launch
import org.jetbrains.compose.resources.stringResource
import org.koin.compose.viewmodel.koinViewModel
import port.movies_ui.generated.resources.Res
import port.movies_ui.generated.resources.retry

@Composable
internal fun MoviesScreen(
    modifier: Modifier = Modifier,
    uiState: LazyPagingItems<Movie>,
    action: (MoviesEvent) -> Unit
) {
    Box(
        modifier = modifier
            .fillMaxSize()
                ...
                items(
                    ...
                    if (movie != null)
                        MovieCardRow(modifier = Modifier, movie = movie, onClick = {
                            action(MoviesEvent.NavigateToDetailsScreen(it))
                        })
                }
        ...
        if (mediatorLoadState?.append is LoadState.Error && mediatorLoadState.hasError && endOfList) {
            LaunchedEffect(uiState.loadState.append) {
                action(MoviesEvent.ShowSnackbar(message = "Loading error") { uiState.retry() })
            }
        }
    }
}
...
fun EntryProviderScope<NavKey>.moviesScreen(
    ...
    entry<Routes.Home>(
        metadata = ListDetailSceneStrategy.listPane {
            Column(
                ...
                Text("Please select one movie")
            }
        }
    ) {
        val moviesViewModel = koinViewModel<MoviesViewModel>()
        val moviesPaginationData =
            moviesViewModel.moviesPagingData.collectAsLazyPagingItems()
        MoviesScreen(
            modifier = Modifier,
            uiState = moviesPaginationData,
            action = { moviesAction ->
                when (moviesAction) {
                    is MoviesEvent.NavigateToDetailsScreen -> {
                        backStack.addDetail(Routes.Details(moviesAction.movieId))
                    }

                    MoviesEvent.OnBackPressed -> moviesViewModel.actionHandler(
                        moviesAction
                    )

                    MoviesEvent.PullToRefresh -> moviesViewModel.actionHandler(
                        moviesAction
                    )

                    is MoviesEvent.DeleteMovie -> moviesViewModel.actionHandler(
                        moviesAction
                    )

                    is MoviesEvent.Init -> moviesViewModel.actionHandler(
                        moviesAction
                    )
                    ...
                    is MoviesEvent.ShowSnackbar -> {
                        coroutineScope.launch {
                            snackbarHostState.showSnackbar(
                                message = moviesAction.message.orEmpty(),
                                actionLabel = "Retry",
                                duration = SnackbarDuration.Short
                            ).apply {
                                when (this) {
                                    SnackbarResult.ActionPerformed -> {
                                        moviesAction.action?.invoke()
                                    }
